...
def default_profile(
    ...
    p = RunnerProfile.from_age(
        age=age, hrrest=hrrest, sex=sex, birth_year=by, lthr_manual=lthr_manual
    )
    p.hr_zones = {
        1: (0, int(0.60 * p.hrmax)),
        2: (int(0.60 * p.hrmax) + 1, int(0.70 * p.hrmax)),
        3: (int(0.70 * p.hrmax) + 1, int(0.80 * p.hrmax)),
        4: (int(0.80 * p.hrmax) + 1, int(0.90 * p.hrmax)),
        5: (int(0.90 * p.hrmax) + 1, p.hrmax),
    }
    return p


def with_estimated_hrmax(
    profile: RunnerProfile, activities, min_recurrence: int = 2, floor: int = 120
) -> RunnerProfile:
